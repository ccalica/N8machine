# GDB Remote Serial Protocol Stub — Implementation Spec for N8machine (v2)
...
All decisions were evaluated across 3 independent agents (A: Protocol, B: Emulator Integration, C: Architecture) and refined by two critique rounds (HW: Hardware, SW: Software). Decisions marked with agreement count.
...
| D16 | Use full prefetch sequence for GDB PC writes, preserving IRQ/NMI | Safe regardless of internal CPU state. **(v2: preserves existing IRQ/NMI pin state per HW-13)** | B,HW |
| D17 | GDB memory reads bypass bus decode; direct `mem[]` access | Non-destructive reads; avoids TTY queue corruption. **(Emulator-only; see Appendix A)** | B,C |
...
Transitions:
  NO_CLIENT -> HALTED     : accept() — halt emulator, align to SYNC
...
| Command | Action |
|---------|--------|
| `reset` | Call `reset()` callback. Respond with hex-encoded "Reset\n" then `OK`. |
...
<memory-map>
  ...
  <memory type="ram"  start="0xC000" length="0x0200">
    <!-- Device-mapped: frame buffer ($C000-$C0FF), TTY ($C100-$C10F) -->
    <!-- Reads via GDB return raw mem[], not device state (emulator only) -->
  </memory>
  <memory type="ram"  start="0xC200" length="0x0E00"/>
...
#### Initial Connection
```
GDB connects -> stub halts emulator, aligns to SYNC boundary
GDB -> $qSupported:...#xx
...
**Contract for `reset()`** (per SW-3.4): Sets M6502_RES pin. The actual reset sequence executes on the next `step_instruction()` or `continue_exec()` call. Caller should be aware that PC will not immediately reflect the reset vector.
...
- **Bus decode note** (per HW-12): `mem[0xC103]` contains the last CPU-written value, not current device state. Device read data overwrites `mem[]` only during a CPU bus read cycle. This is acceptable for GDB inspection but documented as a known limitation.
...
#### T47: Connect halts emulator [D23]
```
...
#### T58: Memory map XML transfer [v2]
```
...
Followed by: $OK#XX
Verify: emulator_reset() was called
```
...
2. Send reset via qRcmd (v2) or callback
3. Continue
...
Action: Set IRQ condition (mem[0x00FF] != 0 or assert IRQ pin)
Send: $c#XX
...
Action: Reset via qRcmd "reset" (or emulator reset button)
Send: $c#XX
...
Setup: bp at IRQ handler entry (read from 0xFFFE/0xFFFF)
       IRQ will fire during normal execution
Send: $c#XX
...
- `qXfer:memory-map:read` (implemented in v2) helps GDB understand which regions are I/O
